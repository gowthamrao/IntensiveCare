...
AS (
	...
	WHERE revenue_code_concept_id IN (
			38003101,38003102,38003103,38003105,38003106,38003107,38003108,38003109,38003110,38003111,38003112,38003113
			)
	),
event_dates
AS (
...
DELETE FROM @target_database_schema.@target_cohort_table where cohort_definition_id = 8;
